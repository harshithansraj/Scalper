...
const solInrMarket = new DepthManager("B-DOT_INR");

const usdtInrMarket = new DepthManager("B-USDT_INR");

const solUsdtMarket = new DepthManager("B-DOT_USDT");
...
async function main() {
    ...
    await createOrder("buy", "SANDINR", (parseFloat(highestBid) + 0.08).toFixed(3), 7, Math.random().toString());
    await new Promise((r) => setTimeout(r, 10000));
    await cancelAll("SANDINR");
